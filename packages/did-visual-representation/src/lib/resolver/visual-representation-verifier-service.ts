...
import {
  ...
  VisualRepresentationObserverApi,
  DidVisualRepresentationStructure,
} from '@trustcerts/observer';

export class VisualRepresentationVerifierService extends VerifierService {
  protected override apis!: VisualRepresentationObserverApi[];
  ...
  protected setEndpoints(id: string) {
    ...
    this.apis = network.observers.map(
      (url) =>
        new VisualRepresentationObserverApi(
          new Configuration({ basePath: url })
        )
    );
  }
  ...
  async getDidDocument(
    id: string,
    config: DidManagerConfigValues<DidVisualRepresentationTransaction>
  ): Promise<VisualRepresentationDocResponse> {
  ...
  async getDidTransactions(
    ...
    validate: boolean,
    time: string
  ): Promise<DidVisualRepresentationStructure[]> {
    this.setEndpoints(id);
    for (const api of this.apis) {
      const res = await api
